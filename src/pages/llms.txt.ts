...
export const GET: APIRoute = async () => {
  ...
  lines.push(`- [Tier list](${SITE}/tier-list/) — five readings of the same models side by side, all re-captured 2026-08-17: the Arena crowd leaderboard (formerly LMArena; eleven boards, each carrying its own publish date because they no longer share one), Artificial Analysis's independent agentic Intelligence Index with per-task cost economics ($/task), the labs' launch-deck claims shown beside the matching public independent board where one exists, Arena's Agent board (a Net Improvement Score, not Elo — kept in its own shape on purpose), and an operator-usefulness ranking you can build and share. As of this capture the most capable model is no longer the most expensive per task — Claude Opus 5 took the top of the independent index below the cost of the model beneath it, which broke a relationship that had held on every prior capture.`);
