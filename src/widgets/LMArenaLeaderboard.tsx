...
import {
  LMARENA, VENDOR_META, LMARENA_SNAPSHOT, LMARENA_MIRROR,
...
//
// Freshness is instead carried honestly: each board renders its OWN vote cutoff
// from `freshness`, because Arena recomputes each on its own cadence and the
// capture date is not a substitute for the published data cutoff.
...
export default function LMArenaLeaderboard() {
  ...
  const top10Spread = Math.max(...top10.map((r) => r.score)) - Math.min(...top10.map((r) => r.score));
  ...
  const css = `
    ...
    .lmb-row{display:grid;grid-template-columns:42px 132px minmax(0,1fr) 110px 72px;align-items:center;gap:12px;padding:9px 24px;border-bottom:1px solid rgb(var(--line) / .5)}
    ...
    .lmb-md{min-width:0;font:400 14px/1.45 var(--font-mono,monospace);color:rgb(var(--fg));overflow-wrap:anywhere;white-space:normal}
    ...
    @media (max-width:680px){.lmb-row{grid-template-columns:28px minmax(0,1fr) 54px;gap:8px;padding:9px 16px}.lmb-vd,.lmb-track,.lmb-fill{display:none}.lmb-head,.lmb-tabs,.lmb-foot,.lmb-callout,.lmb-note{padding-left:16px;padding-right:16px}.lmb-callout,.lmb-note{margin-left:16px;margin-right:16px}}
  `;
      ...
      <div className="lmb-wrap">
        ...
        {leadN >= 4 && (
          <div className="lmb-callout">
            ...
            {top10Spread} points. Elo gaps are specific to the {cat.name} board;
            the spread alone does not establish a statistically resolved ranking or predict performance on your tasks.
          </div>
        )}
        ...
        <div className="lmb-foot">
          <span>
            Captured {cat.source.captured} from Arena's published board. Ten of eleven Elo boards
            were cross-checked against the {LMARENA_SNAPSHOT}{' '}
            <a href={LMARENA_MIRROR} target="_blank" rel="noopener" style={{ color: 'rgb(var(--accent))' }}>
              community mirror
            </a>{' '}
            capture; Image-to-Code is single-sourced. Arena remains the primary source.
          </span>
          <a href={cat.source.url} target="_blank" rel="noopener" style={{ color: 'rgb(var(--accent))', fontWeight: 600 }}>
            Open {cat.name} on arena.ai →
          </a>
        </div>
      </div>
    </div>
