...
[Chapter 25](/chapters/25-evals-or-hope/)'s small <GlossaryTerm term="Eval">eval</GlossaryTerm> checks source health and artifact structure. Those are necessary checks, but they do not establish whether a tutor's correction is accurate. Internal reports need factual checks too; customer-facing replies add task-specific quality requirements.
...
**Generate the dataset.** You describe the task and the inputs your prompt takes; the harness asks the model for a set of *diverse* scenarios (so the test set isn't ten variations of the same easy case) and turns each into a concrete test case. Treat generated cases as candidates, not automatically correct labels. Review the expected answers, include real failure cases, and keep a held-out set separate from prompt tuning. This expands [Chapter 25's golden-set checks](/chapters/25-evals-or-hope/); it does not replace them.
...
Choose the acceptance criteria before running the harness. The companion [AI agent evaluation metrics that matter](https://vladyslavpodoliako.com/thoughts/ai-agent-evaluation-metrics-that-matter) connects task success, human intervention, cost, and failures to an operating scoreboard. Record the dataset version, model configuration, and failed cases alongside the score so the next run can be compared honestly.
