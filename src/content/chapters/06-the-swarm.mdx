...
<Callout type="tip">
The swarm's missing organ is a queue. Once you run more than two sessions, "what should each one work on" stops being a prompt and becomes infrastructure: a board with a definition of ready, issues written for an agent with no memory, and a review fleet that wakes by file path. The full working setup — GitHub Projects, three slash commands, six reviewers, every option judged — is on [the agent-workflow page](/agent-workflow/).
</Callout>
