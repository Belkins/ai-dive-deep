...
**Inputs.** What does this instance need to read to do its job? Be specific. "HubSpot deals" is not specific. "Stage changes since the last successfully processed window, for deals above $10K" is specific. Store the window boundary; a rolling 24-hour query loses coverage after an outage or a weekend.
...
**Failure mode.** A successful read with no changes is a quiet result. A failed read, expired credential, incomplete page of results, or failed delivery is not. Retry within a fixed budget, retain the last successful cursor, and notify the workflow owner once per incident. Repeated failures update the same incident; recovery closes it. Silence should mean "nothing changed," not "we stopped checking."
...
**Trigger:** 5 PM ET, weekdays. Configure the scheduler's timezone as `America/New_York`, not a fixed UTC offset, so daylight saving changes do not move the local delivery time.

**Inputs:** HubSpot stage changes, Gong transcripts, and Slack #sales activity since each source's last successfully processed cursor. Fix the upper boundary at run start and fetch every page. On first use, explicitly choose the starting window; do not pretend earlier history was checked.
...
**Output:** Slack DM to me with the daily ticker. Slack canvas shared with leadership for the deep-dive. Validate the actual artifact before either delivery using the gate in [Chapter 25](/chapters/25-evals-or-hope/).

**Failure:** if a source or delivery fails, keep its cursor unchanged and notify the owner. Resume from that cursor in bounded catch-up windows, including weekends. Advance a window only after complete reads, validation, and confirmed delivery, or a verified no-change result. If the backlog exceeds source retention or the recovery budget, stop and request a reviewed backfill rather than silently jumping to today.

The recovery rules above are the implementation contract, not evidence that every scheduler supplies them automatically. Test a Friday failure followed by a Monday recovery before trusting this workflow unattended.
...
Use a durable run record keyed by workflow and window, plus source event IDs to deduplicate overlapping reads. Serialize concurrent runs for the same workflow. Track each delivery target separately so a successful DM is not sent again when only the canvas failed. A send timeout is ambiguous: reconcile the existing message before retrying. A cursor alone cannot guarantee exactly-once delivery across two systems.
