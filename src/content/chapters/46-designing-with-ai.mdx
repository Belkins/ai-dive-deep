...
A swarm is how you make *one* good decision. A design system is how you stop re-deciding. That distinction is the whole reason the system exists, and it's the most misunderstood thing in this chapter, so let me be concrete about what it actually is on this site.

It's a document — the [design system](/) these pages live inside, reverse-engineered from the live source by a skill whose only job is to read a running site and write down the rules it's already following. It came out the other end as ten principles and a short token list: dark by default; one hot accent, flame, and nothing else fighting it for "look here"; a green that means *the system is talking*, never decoration; monospace for the machine's voice; a 760-pixel reading column; warm neutrals instead of pure gray; and a near-total ban on shadows — there are seven in the entire application. Seven semantic color tokens carry essentially all of it.
...
And because it's a real system and not a brochure, it documents its own decay. The honest version of "we have a design system" includes the backlog of where the system has drifted — and ours is specific about it. The small uppercase label that sits above almost every section, the most-used atom on the site, has been built ad hoc roughly 108 times across nine different sizes and nine different letter-spacings, because for a long time there was no single class for it. A whole numbered color palette sits in the config about 95% unused, kept only as the reference the real tokens were sampled from. That backlog isn't an embarrassment to hide; it's the first fix. A system that can't tell you where it's drifting isn't disciplined — it just isn't measuring. The ones with no backlog are the ones not looking.
...
<ScreenshotPlaceholder
  id="46-designing-with-ai-flicked"
  ratio="1600/900"
  caption="One product, three intents — and three heading typefaces to match."
  note="chaos: a display face (Bricolage Grotesque), light, four neon accents. hype: a sans (Inter), dark, one coral accent. calm: a serif (Fraunces), light, one muted mint. The mint #35E8B8 is the only constant — the brand under three intents. Captured 2026-06-16."/>
...
I'll say it plainly, per the boundary up top: the first-person column below is Claude and Claude Code, because that's what built this site. Everything else is the researched landscape as of mid-2026, and I'll label it that way rather than pretend I ship with all of it.
...
One honest note, because it proves the last section instead of dodging it: **Claude doesn't generate images.** This is a job you reach fully outside Claude for — the Reach frames below were made with a dedicated image model, not Claude. That's the "which model for which job" rule made literal: pick the tool by the job, and label what's yours. The image model is the brush. The art direction is the part that's mine, and it's the only variable that moved.

### Art direction is intent for pixels

I keep a public skill for this — `imagegen-frontend-web`, part of a collection called `taste-skill`. Stripped to what an operator actually does, it's five moves:
...
<ScreenshotPlaceholder
  ...
  ratio="16/9"
  caption="The directed frame — a moon-base hero for Reach by Folderly."
  note="Reach by Folderly is a pre-build concept; this is an art-direction study, not a shipped site. Made with a dedicated image model (not Claude — Claude doesn't generate images), June 2026."/>
...
The intent I locked was a **moon base**, and the metaphor does the work: the moon base is your audience's inbox, the territory you're settling; emails are capsules landing on an R-branded pad marked PRIMARY INBOX. That one image carries the positioning — lands-at-scale is capsules touching down, performs-not-just-delivers is that the pad is *primary*, not merely "arrived." The locked dials: light mode, flat illustration, one muted blue on warm paper, one clean grotesk, capsules-in-transit as the only motion. Then eight sections, each a *different* composition anchor so you can watch the engine work: an image-as-canvas hero, a mini-minimalist trust bar, an off-grid "how it works," a single-gauge metric with no number invented, a docked product panel, an editorial proof split, a volume-tiered plans frame with no prices invented, and a stacked-center closing call to action.

<ScreenshotPlaceholder
  id="46-designing-with-ai-reach-set"
  ratio="1760/600"
  caption="Eight sections, one world — the art direction was the constant; the image model was swapped in."
  note="The full Reach-by-Folderly moon-base set, one frame per section. Generated June 2026 with a dedicated image model (not Claude), held consistent by one reference frame and one brief. A Folderly concept study, not a shipped product."/>

Eight distinct frames, one unmistakable world. The image model never learned anything between them. The art direction did all the holding.

### Steal the skill

The skill is public and MIT-licensed: `github.com/Belkins/taste-skill` — and because it's a collection, the imagery siblings travel together: `imagegen-frontend-web` for sites, `imagegen-frontend-mobile` for app screens, plus `brandkit`, `redesign-skill`, and `image-to-code-skill`. Install it, override the brief, keep the discipline. The repo's one-line pitch is this whole chapter in seven words — it stops the AI from generating boring, generic slop. Because slop was never the model's fault. It's the absence of a decision. Supply the taste, write it down, and the model executes against yours instead of the internet's average. Same move as the design system — a different surface, the same job.
