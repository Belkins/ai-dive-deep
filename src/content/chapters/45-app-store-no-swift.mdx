...
<Callout type="tip" title="The same bet, one domain further out">
This chapter's move — ship in a language you can't read, own the gates the agent can't walk — has now left the screen entirely. Claude designed a 3D-printable museum frame for the bird station's e-ink panel: eight parts as STL + STEP, emitted by 267 lines of Python whose dimensions were read off the vendor's dimensional drawing, with the verification renders projected from the same CAD source. The process, the six mechanical decisions, and the honest caveat (zero grams printed until the calipers agree) are at <a href="/cad-as-code/">CAD-as-code</a>.
</Callout>
