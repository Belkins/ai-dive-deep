...
<ScreenshotPlaceholder
  id="24-tier-list-1"
  caption="My actual desktop"
  note="My actual desktop — show the dock and the apps I have open at 9 AM on a Tuesday. The S-tier reveals itself."/>
...
- **Mythos is pending.** At Code with Claude 2026, Anthropic publicly conceded that an internal unreleased model called Mythos beats Opus 4.7 across the benchmarks they ran. Release window: "soon." Translation — the S-tier model line is queued for a flip. Build code that doesn't pin to a specific Opus version and the upgrade is a one-line change. Pin to `claude-opus-4-7` literally and you're rewriting in three months. Anthropic is also deprecating `claude-sonnet-4` and `claude-opus-4` before June 15, 2026, so anything still on those names needs a sweep regardless.
...
The tier-list widget at the bottom of this chapter (`/tier-list`) is the live version — it gets updated independently as the landscape shifts. Treat the tables below as the May 2026 snapshot; the widget is the receipt for whatever month you're reading this in. The Mythos entry will move when it ships — see [/research-notes](/research-notes) for the running ledger.
