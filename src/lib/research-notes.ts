...
export type ResearchNote = {
  ...
  kind?: 'models' | 'benchmarks' | 'economics' | 'security' | 'method' | 'research' | 'industry'; // index-scan category chip
  title: string;
  source: string;
...
export const RESEARCH_NOTES: ResearchNote[] = [
  {
    ...
    takeaway: [
      ...
      "The structural finding is a pricing event, not a capability one. For as long as these boards have existed, the most capable model was also the priciest. That link broke: Artificial Analysis put Opus 5 at the top of its Intelligence Index at $2.03 per Index task while Fable 5 sat below it at $2.75. At launch, three independent aggregates disagreed about the order — AA had Opus 5 first by 0.83 points against its own stated ±1% interval, Vals AI had Fable 5 first by 0.32 against ±1.35, and Arena's crowd had Fable 5 first with overlapping intervals. All three gaps sat inside their own error bars at the time. The defensible sentence was \"statistically indistinguishable at half the price,\" never \"better.\" (The boards keep moving — the tier list carries the current capture.)",
    ...
    receipts: [
      ...
      { label: 'The hierarchy (launch week)', value: 'AA: Opus 5 by 0.83 (±1%) · Vals AI: Fable 5 by 0.32 (±1.35) · Arena: Fable 5, intervals overlap' },
      ...
      { label: 'Absent from (as of 2026-07-27)', value: 'tbench.ai Terminal-Bench · Scale SWE-bench Pro · official OSWorld 2.0 · Arena Agent board (which it joined, and led, by mid-August)' },
  ...
  },
  {
    ...
    source: 'Berkeley RDI primary report, April 2026; note reviewed 2026-09-05',
    date: '2026-09-05',
    kind: 'benchmarks',
    tagline: 'Protect the evaluator as well as the task output.',
    takeaway: [
      "Berkeley RDI built an automated scanning agent that found exploitable evaluation infrastructure in eight tested agent benchmarks. The demonstrated exploits obtained high scores without solving the intended tasks.",
      "This is an evaluator-security result, not evidence that every published score is fraudulent or that training made every model cheat. It does not establish a universal score discount or validate a particular production model.",
      "Operator interpretation: benchmark exploits, qualitative user interviews, and document-editing experiments answer different questions. They are not three replications of one measured failure rate.",
    ],
    implications: [
      'Keep graders, approved baselines, and held-out answers outside the producing agent\'s write access.',
      'Run private, human-reviewed cases and validate the actual delivery artifact. A smoke test alone does not establish factual correctness.',
      'Inspect benchmark versions and evaluation controls before comparing scores; do not subtract an invented fixed discount.',
    ],
    receipts: [
      { label: 'Tested evaluation environments', value: '8, all exploitable in the reported audit' },
      { label: 'Report date precision', value: 'April 2026; exact publication day not verified' },
      { label: 'Primary source checked', value: '2026-09-05' },
    ],
    chapters: [
      { slug: '25-evals-or-hope', ref: 'Ch 25', why: 'protect the checker and validate the delivered artifact' },
      { slug: '28-failure-receipts', ref: 'Ch 28', why: 'turn failures into explicit regression cases' },
      { slug: '24-tier-list', ref: 'Ch 24', why: 'compare benchmark evidence with private workload results' },
    ],
    links: [
      { label: 'Berkeley RDI primary report', href: 'https://rdi.berkeley.edu/blog/trustworthy-benchmarks-cont/' },
    ],
  },
  {
  ...
  },
  {
    ...
    takeaway: [
      ...
      "Three gates govern the line: privacy review before moving people data, factual checks on collected evidence, and transparent human responsibility. The Anthropic interviews describe reliability concerns, not a measured failure rate for people decisions. A refusal instruction needs testing and does not replace access controls.",
    ],
    implications: [
      ...
      "Aggregation is not automatically safe: limit data access, check source accuracy, obtain the required approvals, and keep a human accountable before shipping even a KPI rollup.",
      "Operator concern, not a measured study result: undisclosed automated review may undermine team trust and alter how people communicate. Agree on the workflow and its limits with the affected team.",
    ],
    receipts: [
      { label: 'Anthropic interview reliability concerns', value: 'Self-reported concerns, not people-workflow failure rates' },
    ...
    chapters: [
      ...
      { slug: '25-evals-or-hope', ref: 'Ch 25', why: 'consequential workflows need reviewed facts and explicit failure cases' },
    ...
    links: [
      { label: 'Anthropic interview study', href: 'https://www.anthropic.com/features/81k-interviews' },
    ],
  },
  {
    title: "Anthropic's 81k interviews — what 80,508 Claude users in 159 countries actually want from AI",
    source: 'Anthropic qualitative interview study; primary source checked 2026-09-05',
    date: '2026-05-13',
    kind: 'research',
    tagline: 'Reported experiences and concerns, not a production incident rate.',
    takeaway: [
      "Anthropic interviewed 80,508 participating Claude users across 159 countries and 70 languages. Claude-powered classifiers categorized their open-ended responses; concerns could receive multiple labels.",
      "Unreliability was the most common concern category at 26.7%. That is a share of interview responses, not a percentage of failed tasks, and not the highest number across every measure in the study.",
      "This voluntary user sample does not prove the playbook's ROI, establish a causal advantage for solo operators, or compare Claude against competing models. It is distinct from benchmark experiments and operational incident records.",
    ],
    implications: [
      'Ask users which outputs they need to verify, then test those workflows against reviewed facts.',
      'Measure time saved and correction work locally before making an ROI claim.',
      'Keep human review for consequential decisions; a concern survey cannot certify a workflow as safe.',
    ],
    receipts: [
      { label: 'Interview participants', value: '80,508' },
      { label: 'Countries / languages', value: '159 / 70' },
      { label: 'Responses coded with unreliability concern', value: '26.7%, not a task-failure rate' },
    ],
    chapters: [
      { slug: '25-evals-or-hope', ref: 'Ch 25', why: 'test factual outputs, not inferred failure rates from interviews' },
      { slug: '26-team-adoption', ref: 'Ch 26', why: 'discuss verification burden and human responsibility' },
      { slug: '19-build-products', ref: 'Ch 19', why: 'validate benefits on the actual user workflow' },
    ],
    links: [
